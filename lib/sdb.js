    ...
    find: function(conditions, callback){
      ...
      Object.keys(c)
        .forEach(function (name){
          ...
          /**
           * First work out the expression and field name, converting:
           *
           *  { 'field': 'val' } to 'field = "val"'
           *  { '=field': 'val' } to 'field = "val"'
           *  { '<field': 'val' } to 'field < "val"'
           *  { '>field': 'val' } to 'field > "val"'
           */
          ...
          /**
           * Next set the value to search for. If we're testing for x = null,
           * then use 'is null':
           */

          var value = c[name];
          if (expression === '=' && value === 'string:null'){
            expression = 'is';
            value = 'null';
          } else {
            value = '"' + value + '"';
          ...
          /**
           * Finally, add the condition as part of the where clause:
           */

          where.push(fieldName + ' ' + expression + ' ' + value);
        });
      ...
      var _select = function (query, override, callback){
        sdb.select(query, override, function (err, res, meta){
          ...
          if (meta.result.SelectResult.NextToken){
            _select(query, {NextToken: meta.result.SelectResult.NextToken},
              callback);
          } else {
